    ...
    <div id="productList" class="d-none">
      <table class="table table-success table-hover table-striped w-100">
        ...
        <c:forEach items="${rs.rows }" var="row" varStatus="status">
          <tr>
            <td>${status.count }</td>
            <td><img class="edit" src="images/edit.png" alt="" onclick="fnc1('${row.product_name }','${row.product_catalog }','${row.product_price }','${row.product_desc }')"></td>
            <td>${row.product_name }</td>
            <td>${row.product_catalog }</td>
            <td>${row.product_price }</td>
            <td>${row.product_stock }</td>
            <td><a href="products.jsp?delid=${row.product_id}" onclick="return delConfirm('${row.product_name}');">刪除</a> </td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
    ...
    <div class="tab-pane fade show active " role="tabpanel"
      ...
      <div style="margin-top: -18px; margin-bottom: -8px">
        <form id="p-add-form" method="post" action="DBinsert.jsp">
          <button type="submit" class="button-54 p-save"><img src="images/search.png" style="width: 20px"
                                                              alt="">新增/儲存</button></input>
          ...
          <table class="table table-hover">
            ...
            <tr>
              ...
              <td>
                <input type="text" name="pdtypeselect" class="pdtypeselect" list="pdtypeselect" value="">
            ...
            <tr>
              ...
              <td><input type="number" name="pdprice" id="pdprice" value=""></td>
              <td scope="row" style="text-align: end;font-weight: bold">產品描述</td>
              <td><textarea form="p-add-form" name="pdesc" id="pdesc" cols="35"
                            wrap="soft" value="產品描述value"></textarea></td>
            </tr>
            <tr>
...
<script>
  function fnc1(pname,ptype,price,pdesc){
